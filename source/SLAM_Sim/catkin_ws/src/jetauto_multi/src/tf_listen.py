...
if __name__ == '__main__':
    ...
    listener = tf.TransformListener() #TransformListener创建后就开始接受tf广播信息，最多可以缓存10s(after TransformListener is created, start receiving tf broadcast message. The maximum cache size is 10s)
    ...
    while not rospy.is_shutdown():
        msg = Twist()
        try:
            (trans, rot) = listener.lookupTransform(base_frame, target_frame, rospy.Time()) #查看相对的tf, 返回平移和旋转(check relative tf, and return translation and rotation)
