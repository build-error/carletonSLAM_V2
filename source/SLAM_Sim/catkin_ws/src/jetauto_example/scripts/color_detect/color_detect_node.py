    ...
    def image_proc(self):
        while self.running:
            if self.image is not None and self.target_colors is not None:
                ...
                img_lab = cv2.cvtColor(self.image, cv2.COLOR_RGB2LAB)  # rgb转lab
                ...
                max_area_rect = 0
                color_area_max_rect = ''    
                areaMaxContour_rect = 0

                max_area_circle = 0
                color_area_max_circle = ''    
                areaMaxContour_circle = 0
                for i in self.target_colors:
                    if self.detect_type[i] == 'line':
                        line_color = i
                        for roi in self.line_roi:
                            ...
                            max_contour_area = common.get_area_max_contour(contours, 30)  # 获取最大面积对应轮廓
                        ...
                        max_contour_area = common.get_area_max_contour(contours, 30)  # 获取最大面积对应轮廓
                        if max_contour_area is not None:
                            area = math.fabs(cv2.contourArea(max_contour_area[0]))
                            if self.detect_type[i] == 'rect':
                ...
                if line_color is not None and int(center_pos) != -1:
                ...
                if areaMaxContour_rect != 0:
                    rect = cv2.minAreaRect(areaMaxContour_rect[0])#最小外接矩形
                    ...
                    pt3_x, pt3_y = box[2, 0], box[2, 1]
                    # 中心点
                    x, y = (pt1_x + pt3_x) / 2, (pt1_y + pt3_y) / 2
                    x = int(x) + self.rect_roi['x_min']
                    y = int(y) + self.rect_roi['y_min']
                    ...
                    color_info.x = x
                    color_info.y = y
                    color_info.angle = rect[2]
                    color_info_list.append(color_info)
                if areaMaxContour_circle != 0:
                    ((x, y), radius) = cv2.minEnclosingCircle(areaMaxContour_circle[0]) # 获取最小外接圆
