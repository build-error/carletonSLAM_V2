...
def draw_rectangle(img, imgpts):
    ...
    cv2.drawContours(img, [imgpts[:4]], -1, (0, 255, 0), -3)  # 绘制轮廓点，填充形式(draw the contour point in fill form)
    for i, j in zip(range(4), range(4, 8)):
        cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), (255), 3)  # 绘制线连接点(draw the line connection point)
        ...
        if self.target_model is not None: 
            ...
            if detections != ():
                for detection in detections: # 遍历(traverse)
                    ...
                    # 使用世界坐标系k个点坐标(OBJP)，对应图像坐标系2D的k个点坐标(corners)，以及相机内参camera_intrinsic和dist_coeffs进行反推图片的外参r, t(Use the world coordinate system k point coordinates (OBJP), the k point coordinates (corners) corresponding to the 2D image coordinate system, and the camera internal parameters camera_intrinsic and dist_coeffs to reverse the external parameters r, t of the picture)
