...
class ObjectTracker:
    ...
    def __call__(self, image, result_image, threshold):
        ...
        image = cv2.cvtColor(image, cv2.COLOR_RGB2LAB)  # RGB转LAB空间(convert RGB into LAB space)
        ...
        mask = cv2.inRange(image, tuple(target_color[1]), tuple(target_color[2])) # 二值化(binaryzation)
        ...
        contour_area = map(lambda c: (c, math.fabs(cv2.contourArea(c))), contours)  # 计算各个轮廓的面积(calculate area of each contour)
        contour_area = list(filter(lambda c: c[1] > 40, contour_area))  # 剔除>面积过小的轮廓(eliminate area that is too small)
        circle = None
...
class OjbectTrackingNode:
    ...
    def image_callback(self, ros_image: Image):
        ...
        result_image = np.copy(rgb_image) #  显示结果用的画面(display result image)
        with self.lock:
            # 颜色拾取器和识别追踪互斥, 如果拾取器存在就开始拾取(Color Picker and Recognition Tracking are mutually exclusive, start picking if the picker exists)
            if self.color_picker is not None:  # 拾取器存在(picker exists)
